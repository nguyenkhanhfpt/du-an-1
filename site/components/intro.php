<section>
    <div class="row py-3 pt-md-0">
        <div class="col-6 col-md-3 mb-2 p-0 px-md-3">
            <div class="overflow-hidden py-xl-5 py-md-3 py-2 px-xl-6 px-md-3 px-3 d-flex flex-column flex-md-row align-items-center">
                <img src="<?=$URL_IMG?>/shipped.svg" class="img_intro" width="35px">
                <div class="pl-md-3 small text-center">
                    <strong class="d-block mb-1">Miễn phí ship</strong>
                    <p class="m-0">Trong khu vực thành phố</p>
                </div>
            </div>
        </div>
        <div class="col-6 col-md-3 mb-2 p-0 px-md-3">
            <div class="overflow-hidden py-xl-5 py-md-3 py-2 px-xl-6 px-md-3 px-3 d-flex flex-column flex-md-row align-items-center">
                <img src="<?=$URL_IMG?>/piggy-bank.svg" class="img_intro" width="35px">
                <div class="pl-md-3 small text-center">
                    <strong class="d-block mb-1">Tiết kiệm tiền</strong>
                    <p class="m-0">Sản phẩm với giá ưu đãi</p>
                </div>
            </div>
        </div>
        <div class="col-6 col-md-3 mb-2 p-0 px-md-3">
            <div class="overflow-hidden py-xl-5 py-md-3 py-2 px-xl-6 px-md-3 px-3 d-flex flex-column flex-md-row align-items-center">
                <img src="<?=$URL_IMG?>/save-money.svg" class="img_intro" width="35px">
                <div class="pl-md-3 small text-center">
                    <strong class="d-block mb-1">Đổi trả miễn phí</strong>
                    <p class="m-0">Nếu sản phẩm có vấn đề</p>
                </div>
            </div>
        </div>
        <div class="col-6 col-md-3 mb-2 p-0 px-md-3">
            <div class="overflow-hidden py-xl-5 py-md-3 py-2 px-xl-6 px-md-3 px-3 d-flex flex-column flex-md-row align-items-center">
                <img src="<?=$URL_IMG?>/support.svg" class="img_intro" width="35px">
                <div class="pl-md-3 small text-center">
                    <strong class="d-block mb-1">Hỗ trợ</strong>
                    <p class="m-0">Nhân viên hổ trợ 24/7</p>
                </div>
            </div>
        </div>
    </div>
</section>